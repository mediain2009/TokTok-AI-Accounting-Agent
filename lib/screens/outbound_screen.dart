...
class _OutboundScreenState extends State<OutboundScreen> {
  ...
  Future<void> _load() async {
    ...
    _rows = await DbHelper.getOutbounds(start: _start, end: _end, warehouseId: _filterWarehouseId);
    setState(() => _loading = false);
  }

  Future<void> _showDialog() async {
    if (_warehouses.isEmpty) { _alert('창고를 먼저 등록해 주세요.'); return; }
    ...
    final customerCtrl= TextEditingController(text: '');
          ...
          content: SizedBox(
            ...
            child: Form(
              ...
              child: Column(mainAxisSize: MainAxisSize.min, children: [
                TextFormField(
                  controller: dateCtrl,
                  decoration: const InputDecoration(labelText: '출고일 *', border: OutlineInputBorder()),
                ...
                TextFormField(
                  ...
                  decoration: const InputDecoration(labelText: '출고처/거래처', border: OutlineInputBorder()),
                ),
                const SizedBox(height: 10),
                TextFormField(
                  controller: noteCtrl,
    ...
    final ob = InventoryOutbound(
      outboundDate: dateCtrl.text,
    ...
    await DbHelper.insertOutbound(ob);
    await _load();

    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('출고가 등록되었습니다.')),
      );
    }
  }

  Future<void> _delete(InventoryOutbound r) async {
    final ok = await showDialog<bool>(
      context: context,
      builder: (ctx) => AlertDialog(
        title: const Text('출고 삭제'),
        content: Text('${r.outboundNo} 출고 내역을 삭제하시겠습니까?'),
        actions: [
          TextButton(onPressed: () => Navigator.pop(ctx, false), child: const Text('취소')),
    ...
    if (ok == true) {
      await DbHelper.deleteOutbound(r.id!);
      await _load();
    }
  }
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          Row(children: [
            Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              const Text('출고 관리', style: TextStyle(fontSize: 22, fontWeight: FontWeight.bold)),
              const SizedBox(height: 4),
              Text('창고별 출고 내역을 기록합니다.', style: TextStyle(color: Colors.grey[600], fontSize: 13)),
            ]),
            const Spacer(),
            ElevatedButton.icon(
              onPressed: _showDialog,
              icon: const Icon(Icons.remove_circle_outline),
              label: const Text('출고 등록'),
          ...
          if (!_loading) Row(children: [
            _statCard('총 출고건', '${_rows.length}건', Colors.red),
            const SizedBox(width: 12),
            _statCard('총 수량', '${_fmt(_totalQty)}', Colors.orange),
            const SizedBox(width: 12),
            _statCard('총 금액', '${_fmt(_totalAmt)}원', Colors.deepPurple),
          ]),
          const SizedBox(height: 12),
          Expanded(
            child: _loading
                ...
                : _rows.isEmpty
                    ? Center(child: Text('출고 내역이 없습니다.', style: TextStyle(color: Colors.grey[500])))
                    : Card(
                        child: SingleChildScrollView(
                          child: DataTable(
                            columnSpacing: 16,
                            columns: const [
                              DataColumn(label: Text('출고번호')),
                              DataColumn(label: Text('출고일')),
                              ...
                              DataColumn(label: Text('출고처')),
                              DataColumn(label: Text('관리')),
                            ],
                            rows: _rows.map((r) => DataRow(cells: [
                              DataCell(Text(r.outboundNo, style: TextStyle(color: Colors.grey[600], fontSize: 12))),
                              DataCell(Text(r.outboundDate)),
                              DataCell(Text(r.warehouseName, style: const TextStyle(fontWeight: FontWeight.w500))),
                              ...
                              DataCell(Text(r.customer.isNotEmpty ? r.customer : '-')),
                              DataCell(IconButton(
